...
[*] '/home/foo1/Desktop/kknock/pwnable/FSB_EX/FSB_EX'
    ...
    PIE:      No PIE (0x400000)
```
이 문제도 이전에 x86 아키택처와 동일한 방식으로 풀면되지만 몇 가지 문제가 있음

일단 표현을 위해 8바이트가 사용됨, 최소 6바이트
...
shell()의 어셈블리 코드는 아래와 같음
```sh
pwndbg> disassemble shell 
Dump of assembler code for function shell:
   0x0000000000401166 <+0>:	push   rbp
   0x0000000000401167 <+1>:	mov    rbp,rsp
   0x000000000040116a <+4>:	lea    rax,[rip+0xe93]        # 0x402004
   0x0000000000401171 <+11>:	mov    rdi,rax
   0x0000000000401174 <+14>:	call   0x401040 <system@plt>
   0x0000000000401179 <+19>:	nop
   0x000000000040117a <+20>:	pop    rbp
   0x000000000040117b <+21>:	ret    
End of assembler dump.
```
여기서 일반적으로 shell()을 사용한다고 하면, 당연히 `0x401166` 의 주소를 사용하는 경우가 많음
그러나 스택정렬을 해결하기 위해서 접근을 다르게 할 필요가 있음
스택정렬 이슈가 나왔을 땐, 많은 경우 스택의 위치가 `+0x08`되어야 하거나 `-0x08`가 되어야 함
이를 해결하기 위해선 pop이나 push가 있는 가젯을 사용할 수도 있겠지만 
지금처럼 상황이 받쳐주지 않는다면, `push rbp` 등을 생략한 후 함수의 위치로 이동하여 해결할 수 있음
